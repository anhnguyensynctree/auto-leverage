...
describe("output-cache", () => {
  ...
  it("1 — hit: Redis GET returns cached value → getCached returns result", async () => {
    const { getCached } = await import("../output-cache");
    mockGet.mockResolvedValueOnce(CACHED_VALUE);

    const result = await getCached("output:comp-a,comp-b:growth");

    expect(mockGet).toHaveBeenCalledOnce();
    expect(mockGet).toHaveBeenCalledWith("output:comp-a,comp-b:growth");
  ...
  it("2 — miss: Redis GET returns null → getCached returns null", async () => {
    ...
    const result = await getCached("output:comp-a:miss");
  ...
  it("3 — Redis GET throws → getCached returns null without propagating", async () => {
    ...
    await expect(getCached("output:comp-a:err")).resolves.toBeNull();
  });

  it("4 — Redis SET throws → setCached is a silent no-op", async () => {
    const { setCached } = await import("../output-cache");
    mockSet.mockRejectedValueOnce(new Error("set error"));

    await expect(
      setCached("output:comp-a:seterr", CACHED_VALUE),
